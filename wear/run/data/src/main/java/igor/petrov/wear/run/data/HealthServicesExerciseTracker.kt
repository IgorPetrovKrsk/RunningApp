...
class HealthServicesExerciseTracker(
        ...
        get() = callbackFlow {
            val callback = object : ExerciseUpdateCallback {
                ...
                override fun onExerciseUpdateReceived(update: ExerciseUpdate) {
                    val hearRates = update.latestMetrics.getData(DataType.HEART_RATE_BPM)
                    val currentHeartRate = hearRates.firstOrNull()?.value
    ...
    override suspend fun resumeExercise(): EmptyResult<ExerciseError> {
        ...
        val result = getActiveExerciseInfo()
        if (result is Result.Error && result.error == ExerciseError.ONGOWING_OTHER_EXERCISE) {
            return result
        }
    ...
    private suspend fun getActiveExerciseInfo(): EmptyResult<ExerciseError> {
        ...
        return when (info.exerciseTrackedStatus) {
            ...
            ExerciseTrackedStatus.OWNED_EXERCISE_IN_PROGRESS -> Result.Error(ExerciseError.ONGOWING_OWN_EXERCISE)
            ExerciseTrackedStatus.OTHER_APP_IN_PROGRESS -> Result.Error(ExerciseError.ONGOWING_OTHER_EXERCISE)
